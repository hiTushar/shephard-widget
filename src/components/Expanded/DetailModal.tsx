...
import { Asset, GroupAsset } from "../../Types"
import AlertTypeData from '../alertTypeData.json';
import './DetailModal.css'

const PATH_MAX_COORDS = { xMin: -50, xMax: 150, yMin: 10, yMax: 100 };

const DetailModal: React.FC<Asset & { alertId?: string, xPos?: number, yPos?: number }> = (data) => {
    const [isOpen, setIsOpen] = useState<boolean>(false);
    const [currentData, setCurrentData] = useState<Asset & { alertId?: string, xPos?: number, yPos?: number }>();
    ...
    const drawLine = (data: GroupAsset & { alertId?: string, xPos?: number, yPos?: number }) => {
        ...
        lineRef.current?.setAttribute('d', `M ${data.xPos!} ${data.yPos!} L ${modalPosX - 5} ${modalPosY} L ${modalPosX} ${modalPosY}`); // that +-1 is the approx correction make the line not overlap the digits, not going into trignometric calculations
        ...
        isOpen && (
            ...
            >
                ...
                <div className="detail-modal__box" ref={modalRef}>
                    <DrawBorder />
                    <Section section='Machine ID' text={currentData!.machine_uuid!} />
                    <Section section='Machine Name' text={currentData!.machine_name!} />
